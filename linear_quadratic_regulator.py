...
def lqr(x_gen, u_gen):
    ...
    x_regulator[:, 0] = x_gen[:,0]*(1 + initial_perturbation)
    x_natural_evolution [:,0] = x_gen[:,0]*(1 + initial_perturbation)
    x_LQR_evolution[:,0] = x_gen[:,0]*(1 + initial_perturbation)
    ...
    return x_LQR_evolution, u_regulator
